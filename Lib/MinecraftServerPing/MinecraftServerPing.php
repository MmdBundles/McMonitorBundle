...
class MinecraftServerPing
{
	...
	public function Connect( )
	{
		...
		if( !$this->Socket )
		{
			throw new MinecraftServerPingException( "Failed to connect or create a socket: $errno ($errstr)" );
		}
	...
	public function Query( )
	{
		...
		do
		{
			...
			{
				throw new MinecraftServerPingException( 'Server read timed out' );
			}
			...
			if (!$block)
			{
				throw new MinecraftServerPingException( 'Server returned too few data' );
			}
		...
		if( $Data === FALSE )
		{
			throw new MinecraftServerPingException( 'Server didn\'t return any data' );
		}
		...
		if( JSON_Last_Error( ) !== JSON_ERROR_NONE )
		{
			...
			{
				throw new MinecraftServerPingException( JSON_Last_Error_Msg( ) );
			}
			else
			{
				throw new MinecraftServerPingException( 'JSON parsing failed' );
			}
	...
	private function ReadVarInt( )
	{
		...
		while( true )
		{
			...
			if( $j > 5 )
			{
				throw new MinecraftServerPingException( 'VarInt too big' );
			}
